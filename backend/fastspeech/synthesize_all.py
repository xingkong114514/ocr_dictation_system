...
from fastspeech.text_normalization import text_normalization, PUNCTRUATION, POLY_PHRASE, NUM_DICT
import jieba
from pypinyin_dict.phrase_pinyin_data import large_pinyin
from fastspeech.transformer.ProsodyModel import CharEmbedding
import os
...
class SpeechSynthesis(object):
    ...
    def _read_internal_config(self, config_dir):
        ...
        preprocess_config = yaml.load(open("fastspeech"+preprocess_config_path, "r"), Loader=yaml.FullLoader)
        model_config = yaml.load(open("fastspeech"+model_config_path, "r"), Loader=yaml.FullLoader)
        train_config = yaml.load(open("fastspeech"+train_config_path, "r"), Loader=yaml.FullLoader)
        configs = (preprocess_config, model_config, train_config)
        return configs, preprocess_config, model_config, train_config

    def text2speech(self, text, save_path, pitch_control=1.0, energy_control=1.0, duration_control=1.5):
...
def gen(text):
    ...
    tts.text2speech(text, save_path="./result")
    if not os.path.exists(f"./result/{text}.wav"):
