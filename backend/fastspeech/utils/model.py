...
from fastspeech import hifigan
from fastspeech.model.fastspeech2 import FastSpeech2
from fastspeech.model.optimizer import ScheduledOptim
...
def get_vocoder(config, device):
    ...
    if name == "MelGAN":
        ...
        with open("fastspeech/"+"hifigan/config.json", "r") as f:
            config = json.load(f)
        config = hifigan.AttrDict(config)
        vocoder = hifigan.Generator(config)
        if speaker == "LJSpeech":
            ckpt = torch.load("fastspeech/"+"hifigan/generator_LJSpeech.pth.tar", map_location=device)
        elif speaker == "universal":
            ckpt = torch.load("fastspeech/"+"hifigan/generator_universal.pth.tar", map_location=device)
